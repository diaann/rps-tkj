...
function buildRpsObjectFromExtraction(extraction, options = {}) {
  ...
  subCpmks.forEach(sub => {
    ...
    rpsData[`sub_cpmk[${cpmkId}][${localIndex}][bentuk_pembelajaran]`] = [sub.bentuk_pembelajaran, sub.metode_pembelajaran].filter(Boolean).join(', ');
    rpsData[`sub_cpmk[${cpmkId}][${localIndex}][strategi_pembelajaran]`] = '';
